...
export const websiteSchema = z.object({
  ...
  domainRating: z.coerce.number().min(0, "Domain rating must be between 0-100").max(100).default(0),
  backlinks: z.coerce.number().min(0, "Backlinks must be positive").default(0),
  referringDomains: z.coerce.number().min(0, "Referring domains must be positive").default(0),
...
export const gameSchema = z.object({
  ...
  gameIcon: z.array(z.string()).optional(),
  gameThumb: z.array(z.string()).optional(),
  gameDeveloper: z.string().optional(),
  gamePublishYear: z.coerce.number().min(1980, "Year must be after 1980").max(new Date().getFullYear(), "Year cannot be in the future").optional().or(z.literal("")),
  gameControls: z.object({
...
export const textlinkSchema = z.object({
  ...
  anchorText: z.string().min(1, "Anchor text is required").max(255, "Anchor text too long"),
  ...
  (data) => data.websiteId || data.customDomain,
  {
...
export const cloudflarePurgeSchema = z.object({
  ...
  (data) => {
    if (data.mode === "url") {
      return data.payload.every(url => z.string().url().safeParse(url).success)
    }
    return true
  },
  {
